...
export function AddressFields() {
  const { register, watch, setValue } = useFormContext<Step3Values>();
  ...
  useEffect(() => {
    const addr = buildAddress({
      viaTipo, viaNumero, viaLetra, viaComp,
    //   numero, letra, compNum, comp2, num2, apto,
      numero, letra, viaComp2, compNum, comp2, apto,
    });
    setValue('address', addr, { shouldValidate: true });
//   }, [viaTipo, viaNumero, viaLetra, viaComp, numero, letra, compNum, comp2, num2, apto, setValue]);
  }, [viaTipo, viaNumero, viaLetra, viaComp, numero, letra, viaComp2, compNum, comp2, apto, setValue]);
    ...
    <div className="grid gap-3">
      ...
      <div className="grid sm:grid-cols-1 md:grid-cols-6 lg:grid-cols-6 gap-2">
        <select className="input" {...register('viaTipo')}>
          {VIA_TIPOS.map(v => <option key={v} value={v}>{v}</option>)}
        </select>

        <input className="input" {...register('viaNumero')} placeholder="47"
               inputMode="numeric" pattern="[0-9]*" />

        <select className="input" {...register('viaLetra')}>
          {LETRA_OPTIONS.map(v => <option key={v} value={v}>{v}</option>)}
        </select>

        <select className="input" {...register('viaComp')}>
          {COMP1_OPTIONS.map(v => <option key={v} value={v}>{v}</option>)}
        </select>

        <input className="input" {...register('numero')} placeholder="61"
               inputMode="numeric" pattern="[0-9]*" />
      ...
      <div className="grid sm:grid-cols-1 md:grid-cols-6 lg:grid-cols-6 gap-2">
        ...
        <input className="input" {...register('compNum')} placeholder="30"
               inputMode="numeric" pattern="[0-9]*" />
        ...
        <input className="input col-span-2" {...register('apto')} placeholder="apto 302" />
      </div>
